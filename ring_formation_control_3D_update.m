...
parent_dir = 'uav_ring_formation3D_update'; % 父目录
if ~exist(parent_dir, 'dir') % 如果父目录不存在，则创建
    mkdir(parent_dir);
    disp(['已创建父目录: ', parent_dir]);
end

% 查找以 frames_ 开头的目录
frame_dirs = dir(fullfile(parent_dir, 'frames_*'));
frame_dirs = frame_dirs([frame_dirs.isdir]); % 仅保留目录
frame_count = numel(frame_dirs); % 获取现有子文件夹的数量
new_frame_dir = fullfile(parent_dir, sprintf('frames_%d', frame_count + 1)); % 创建新的子文件夹名称

% 创建新文件夹，并检查是否成功
[status, msg, msgID] = mkdir(new_frame_dir);
if ~status
    error('无法创建目录 "%s"。错误信息: %s', new_frame_dir, msg);
else
    disp(['已创建新文件夹: ', new_frame_dir]);
end

%% 起飞动画
figure('Position', [100, 100, 1000, 800]);
set(gcf,'Color','w'); % 背景保存为白色
legend_handle = [];
for t = 1:30 % 起飞阶段，30步
    clf;
